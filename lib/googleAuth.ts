...
function requireEnvironmentVariable(name: 'GOOGLE_CLIENT_ID' | 'GOOGLE_CLIENT_SECRET'): string {
...
export function getGoogleRedirectUri(requestOrigin: string): string {
  ...
  return `${configuredAppUrl || requestOrigin}/api/auth/google/callback`;
}

export function createGoogleAuthorizationRequest(redirectUri: string) {
...
export async function exchangeGoogleCodeForIdentity(params: {
  ...
  const clientId = requireEnvironmentVariable('GOOGLE_CLIENT_ID');
  const clientSecret = requireEnvironmentVariable('GOOGLE_CLIENT_SECRET');
  ...
  if (payload.nonce !== params.nonce) {
    throw new Error('Google authentication nonce did not match.');
  }
