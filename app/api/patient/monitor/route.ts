...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!/^PT-[A-Z0-9]{5,12}$/.test(patientId)) {
      return NextResponse.json<ApiResponse>(
        {
          success: false,
          error:
    ...
    if (monitoringRequest?.status === 'pending') {
      return NextResponse.json<ApiResponse>(
        {
          success: false,
          error:
    ...
    if (monitoringRequest?.status === 'approved') {
      return NextResponse.json<ApiResponse>(
        {
          success: false,
          error:
    ...
    await Notification.deleteMany({
      ...
      monitoringRequestId: monitoringRequest._id,
    });
    ...
    return NextResponse.json<ApiResponse>(
      {
        ...
        data: {
          requestId: monitoringRequest._id.toString(),
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!patient || patient.role === 'family') {
      return NextResponse.json<ApiResponse>(
        {
          success: false,
          error:
    ...
    if (!monitoringRequest) {
      return NextResponse.json<ApiResponse>(
        {
          success: false,
          error: 'Monitoring request not found.',
        },
        {
          status: 404,
        }
    ...
    monitoringRequest.respondedAt = new Date();
    ...
    await Notification.deleteMany({
      userId: monitoringRequest.familyId,
      monitoringRequestId: monitoringRequest._id,
      type: {
        $in: [
          'monitoring_approved',
