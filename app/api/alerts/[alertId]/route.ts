...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json<ApiResponse>({ success: true, data: serializeAlert(alert) });
  } catch (error) {
    console.error('[GET /api/alerts/[alertId]]', error);
    return NextResponse.json<ApiResponse>(
...
export async function PATCH(
  ...
  try {
    ...
    const body = (await request.json()) as { action?: 'read' | 'acknowledge' };
    if (!body.action || !['read', 'acknowledge'].includes(body.action)) {
      return NextResponse.json<ApiResponse>(
        { success: false, error: 'action must be read or acknowledge.' },
        { status: 400 },
      );
    }
    ...
    const patientIds = await getApprovedPatientIdsForMonitor(auth.userId);
    const now = new Date();
    const update = body.action === 'acknowledge'
      ? {
          status: 'ACKNOWLEDGED',
          isRead: true,
          readAt: now,
    ...
    console.error('[PATCH /api/alerts/[alertId]]', error);
    return NextResponse.json<ApiResponse>(
      { success: false, error: 'Internal server error' },
      { status: 500 },
    );
