...
export default function AdherenceCard() {
  ...
  useEffect(() => {
    fetchAdherence();
    // Trigger caregiver push if High/Moderate
    fetchAdherence().then(() => {
      if (data && (data.riskLevel === 'High' || data.riskLevel === 'Moderate')) {
        sendRiskNotification(data.riskLevel, data.meanAdherence);
      }
    });
  }, [fetchAdherence]);
  ...
  const trendIcon = data.weeklyTrend === 'improving' ? '↑' : data.weeklyTrend === 'declining' ? '↓' : '→';
  const trendColor = data.weeklyTrend === 'improving' ? 'text-green-600' : data.weeklyTrend === 'declining' ? 'text-red-600' : 'text-gray-500';
    ...
    <div className={`rounded-lg border p-6 ${riskColors[data.riskLevel]}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
        <div className="bg-white/60 rounded-lg p-3 text-center">
          <p className="text-xs text-gray-500 mb-1">Mean Rate</p>
          <p className="text-2xl font-bold text-gray-900">{data.meanAdherence}%</p>
        </div>
        <div className="bg-white/60 rounded-lg p-3 text-center">
          <p className="text-xs text-gray-500 mb-1">Median Rate</p>
          <p className="text-2xl font-bold text-gray-900">{data.medianAdherence}%</p>
        </div>
        <div className="bg-white/60 rounded-lg p-3 text-center">
        ...
        <div className="bg-white/60 rounded-lg p-3 text-center">
          <p className="text-xs text-gray-500 mb-1">Perfect days</p>
          <p className="text-2xl font-bold text-gray-900">{data.perfectDays}/7</p>
        </div>
      </div>
