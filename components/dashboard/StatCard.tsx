...
export default function StatCard({ title, value, subtitle, className }: Readonly<StatCardProps>) {
  const hasCustomBg = className?.includes('bg-');
  const baseClasses = hasCustomBg ? 'rounded-[28px] p-6 shadow-sm border border-border/80' : 'bg-card rounded-[28px] p-6 shadow-sm border border-border/80';
