...
const MedicineModal: React.FC<MedicineModalProps> = ({ isOpen, onClose, onSave, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setScheduledTimes(initialData.scheduledTimes.length > 0 ? initialData.scheduledTimes : ["08:00 AM"]);
  ...
  const updateTime = (i: number, v: string) => setScheduledTimes(prev => prev.map((t, idx) => idx === i ? v : t));
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            <Input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Aspirin" disabled={saving} />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Dosage <span className="text-red-500">*</span></label>
            <Input value={dosage} onChange={e => setDosage(e.target.value)} placeholder="e.g. 100mg" disabled={saving} />
          </div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Scheduled Times <span className="text-red-500">*</span></label>
            <div className="space-y-2">
              {scheduledTimes.map((time, index) => (
                <div key={index} className="flex items-center gap-2">
                  <Input value={time} onChange={e => updateTime(index, e.target.value)} placeholder="e.g. 8:00 AM" disabled={saving} className="flex-1" />
                  {scheduledTimes.length > 1 && (
                    <button type="button" onClick={() => removeTime(index)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg" disabled={saving}>
            ...
            <button type="button" onClick={addTime} disabled={saving} className="mt-2 flex items-center gap-1 text-sm text-blue-600 hover:text-blue-700 font-medium">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="e.g. Take with food" disabled={saving} rows={2}
